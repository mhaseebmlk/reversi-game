...
// MONGODB STUFF
const url = 'mongodb://localhost:27017/test1_reversi'
const Promise = require('bluebird')
const MongoClient = Promise.promisifyAll(require('mongodb')).MongoClient

// SET THE CLIENT_COUNT TO zero if it is not already defined
MongoClient.connect(url, (err, db) => {
    if (err) {
        console.log('Unable to connect to the mongoDB server. Error:', err)
    } else {
        console.log('Connection established to', url)
        const clntCntsColectn = db.collection('clientCnts')
        // now read the cliient counts document from this collection
        clntCntsColectn.findOne({name: 'clientsCount'}, (err, res) => {
            if (res) {
                console.log('The clientsCount already exists.')
                //Close connection
                db.close()
            } else {
                const c = {name: 'clientsCount', count: 0}
                clntCntsColectn.insert([c], (err) => {
                    if (err) {
                        console.error(err)
                    }
                    //Close connection
...
const server = http.createServer((request, response) => {
    if (request.url === '/p3client.js') {
        fs.readFile('p3client.js', 'utf-8', (err, data) => {
        ...
        fs.readFile('p3client.html', 'utf-8', (err, data) => {
...
let waitingPiD = undefined
let secondPiD = undefined
...
function setClientID(s) {
    s.on('clientID', cID => {
        if (cID === zero - one) {
            getCurrClientCnt().then(c => {
                updateClientCnt(c)
                // now update the value of the count
                s.emit('setClientID', c + one)
            ...
            console.log('This client has client id = ' + cID)
        }
    })
}
function updateClientCnt(c) {
    MongoClient.connectAsync(url)
    .then(db => {
        console.log('Connection established to', url + 'to get the currClntCnt')
        const clntCntsColectn = db.collection('clientCnts')
        clntCntsColectn.updateOne(
            { name: 'clientsCount' },
            { $set: {count: c + one } }, (err) => {
                if (err) {
                    console.error(err)
...
function getCurrClientCnt() {
    return new Promise (function (resolve) {
        MongoClient.connectAsync(url)
        .then(db => {
            const clntCntsColectn = db.collection('clientCnts')
            clntCntsColectn.findAsync({name: 'clientsCount'})
            .then(cursor => {
...
function createNewGameOrWait(socket) {
    ...
    if (waitingPSocket === undefined) {
        ...
        //         waitingPSocket = undefined
        //     } else {
        //         console.log('XXX COULD NOT CREATE NEW GAME ON DB! XXX')
        //     }
        // })
    }
}
// function createNewGameOnDB(game) {
//
//     console.log('The waitingPiD is: ' + waitingPiD)
//     console.log('The secondPiD is: ' + secondPiD)
//
// //     return new Promise (function (resolve, reject) {
// //         console.log(getClientID(game.p1Socket))
//         // .then(p1Id => {
//         //     console.log(p1Id)
//             // getClientID(game.p2Socket)
//             // .then(p2Id => {
//             //     console.log('The two IDs are: ' + p1Id + ' and ' + p2Id)
//             //     resolve(true)
//             // })
//         // })
//     // })
...
function onMove(s) {
    s.on('move', p => {
        if (s === waitingPSocket) {
            ...
            game.board = updateBoard(game.board, p[one], hasTurn(s)[one])
            updateScores(game)
            switchTurns(s, game)
            emitToPlayers(game)
            checkWinCondition(game)
            countAllValidMoves(game)
        } else {
            s.emit('wrongTurn')
        }
    })
